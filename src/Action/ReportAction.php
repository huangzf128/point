...
final class ReportAction extends BaseAction
{
	...
	public function print(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
		...
		if ($parsedBody['addData']) {
			...
			if ($reportType == 1) {
				...
				$fileName = '納品書&受領書&請求書&送り状_'.$createTime.'.xlsx';
	...
	private function estimate($datas, $customers) {
		...
		foreach ($datas as $row) {
            ...
            $worksheet->setCellValue('A'.($rowNo), $no++);
	...
	private function delivery($datas, $customers) {
		$reader = new XlsxReader();
		$spreadsheet = $reader->load($this->excelRoot.'納品書&受領書&請求書&送り状.xlsx');
		...
		$worksheet3 = $spreadsheet->getSheetByName('請求書');
		$worksheet4 = $spreadsheet->getSheetByName('送り状');
		...
		if ($i > 0 && ($i + 1 - 1) % 20 > 0) {
			...
			$worksheet1->getPageSetup()->setPrintArea('A1:M'.$rowNo);
			$worksheet2->getPageSetup()->setPrintArea('A1:M'.$rowNo);
			$worksheet3->getPageSetup()->setPrintArea('A1:J'.$rowNo);
			$worksheet4->getPageSetup()->setPrintArea('A1:J'.$rowNo);
		}
